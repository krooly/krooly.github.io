    ...
    $.Method({Static:true, Public:true }, "IndexOfAny", 
      ...
      function (str, chars, startIndex) {
        ...
        for (var i = 0; i < chars.length; i++) {
          var index = str.indexOf(chars[i], startIndex);
...
JSIL.ImplementExternals("System.Text.RegularExpressions.Regex", function ($) {
  ...
  var makeRegex = function (pattern, options) {
      ...
      var optionsValue = options.value != undefined ? options.value : options;
      //note: doing the following instead of directly using tRegexOptions.ECMAScript (or the other ones further in this method) makes us avoid a random "number expected" error that happens on IE after passing through this method a certain amount of time.

      var regexOptionsValue = tRegexOptions.ECMAScript.value != undefined ? tRegexOptions.ECMAScript.value : tRegexOptions.ECMAScript;
      if ((optionsValue & regexOptionsValue) === 0) {
          JSIL.RuntimeError("Non-ECMAScript regexes are not currently supported.");
      }

      var flags = "g";
      var regexOptionsIgnoreCaseValue = tRegexOptions.IgnoreCase.value != undefined ? tRegexOptions.IgnoreCase.value : tRegexOptions.IgnoreCase;

      if ((optionsValue & regexOptionsIgnoreCaseValue) !== 0) {
          flags += "i";
      }

      var regexOptionsMultilineValue = tRegexOptions.Multiline.value != undefined ? tRegexOptions.Multiline.value : tRegexOptions.Multiline;
      if ((optionsValue & regexOptionsMultilineValue) !== 0) {
          flags += "m";
      }
